...
def test_nir_fiber_bundle_annotation(
    with_crosshairs, fits_file, fits_center, check_finder
):
    """Test the NIR fiber bundle annotation."""
    finder_chart = FinderChart(fits_file)
    bundle_annotation = fiber_bundle_annotation(
        center=fits_center,
        min_width=2.5 * u.arcmin,
        max_width=5 * u.arcmin,
        max_height=4 * u.arcmin,
        with_crosshairs=with_crosshairs,
