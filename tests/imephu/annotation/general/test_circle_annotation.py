...
def test_circle_annotation_rotated(pivot, angle, fits_file, check_finder):
    ...
    finder_chart = FinderChart(fits_file)
    circle_annotation = CircleAnnotation(SkyCoord(ra="00h40m00s", dec="-60d00m00s"), 15 * u.arcsec, wcs=finder_chart.wcs, edgecolor="none", facecolor="gray")
...
def test_circle_annotation_translated(displacement, fits_file, fits_center, check_finder):
    """Test translated circle annotations."""
    s = f"Some text"
    finder_chart = FinderChart(fits_file)
    circle_annotation = CircleAnnotation(
        fits_center, 15 * u.arcsec, wcs=finder_chart.wcs, edgecolor="none", facecolor="gray"
    )
