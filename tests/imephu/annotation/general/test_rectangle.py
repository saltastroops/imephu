...
def test_rectangle_annotation(fits_file, check_finder):
    ...
    rectangle_annotation = RectangleAnnotation(
        ...
        width=150 * u.arcsec, height=2 * u.arcmin, wcs=finder_chart.wcs, edgecolor="none", facecolor="blue", alpha=0.2
    )
...
def test_rectangle_annotation_rotated(angle, fits_file, fits_center, check_finder):
    finder_chart = FinderChart(fits_file)
    rectangle_annotation = RectangleAnnotation(fits_center, width=300 * u.arcsec, height=150 * u.arcsec, wcs=finder_chart.wcs, edgecolor="none", facecolor="gray", alpha=0.2)
...
def test_rectangle_annotation_translated(displacement, fits_file, fits_center, check_finder):
    finder_chart = FinderChart(fits_file)
    rectangle_annotation = RectangleAnnotation(fits_center, width=100 * u.arcsec, height=200 * u.arcsec, wcs=finder_chart.wcs, edgecolor="none", facecolor="gray", alpha=0.2)
