...
def ephemeris(epoch, ra=0 * u.deg, dec=0 * u.deg):
    return Ephemeris(epoch=epoch, position=SkyCoord(ra=ra, dec=dec), magnitude_range=MagnitudeRange(bandpass="V", min_magnitude=13, max_magnitude=13))
...
def test_for_time_interval_start_must_be_timezone_aware():
    ...
    ephemerides = [ephemeris(datetime(2022, 2, 16, 12, 0, 0, 0, tzinfo=timezone.utc)),
                   ephemeris(datetime(2022, 2, 2, 12, 0, 0, 0, tzinfo=timezone.utc))]
    with pytest.raises(ValueError) as excinfo:
        g = FinderChart.for_time_interval(start, end, ephemerides, 5 * u.arcmin, cast(Any, lambda x: 42))
        next(g)
...
@pytest.mark.parametrize("start,end,expectation", [
...
def test_for_time_interval_start_must_be_earlier_than_end(start, end, expectation):
    ...
    ephemerides = [ephemeris(datetime(2022, 2, 15, 0, 0, 0, 0, tzinfo=timezone.utc)),
                   ephemeris(datetime(2022, 2, 20, 12, 0, 0, 0, tzinfo=timezone.utc))]
    with expectation:
        g = FinderChart.for_time_interval(start, end, ephemerides, 5 * u.arcmin, cast(Any, lambda x: 42))
        next(g)
...
@pytest.mark.parametrize("max_track_length,expectation", [(-1 * u.arcmin, pytest.raises(ValueError)), (0 * u.arcmin, pytest.raises(ValueError)), (1 * u.arcmin, _does_not_raise())])
def test_for_time_interval_max_track_length_must_be_positive(max_track_length, expectation):
    ...
    with expectation as excinfo:
        g = FinderChart.for_time_interval(start, end, ephemerides, max_track_length, cast(Any, lambda x: 42))
        next(g)


@pytest.mark.parametrize("ras,decs", [
    # For a declination of 60 degrees the actual angle on the sky is half the difference
    # in right ascension.
...
def test_for_time_interval_requires_sufficiently_close_ephemeris_positions(ras, decs):
    ...
    ephemerides = [ephemeris(t + timedelta(hours=i), ra=ras[i], dec=decs[i]) for i, _ in enumerate(ras)]
    start = ephemerides[0].epoch
    end = ephemerides[-1].epoch
    with pytest.raises(ValueError):
        g = FinderChart.for_time_interval(start, end, ephemerides, 1 * u.deg, cast(Any, lambda x: 42))
        next(g)


@pytest.mark.parametrize("positions,expected_indices", [
    # For a declination of 60 degrees the actual angle on the sky is half the difference
    # in right ascension.
    (((0 * u.deg, 60 * u.deg), (0.6 * u.deg, 60 * u.deg), (1.8 * u.deg, 60 * u.deg), (2.1 * u.deg, 60 * u.deg), (3.9 * u.deg, 60 * u.deg), (4 * u.deg, 60 * u.deg)),
     ((0, 1, 2), (2, 3), (3, 4, 5))),
    (((0 * u.deg, 0 * u.deg), (0 * u.deg, 0.8 * u.deg), (0 * u.deg, 1.2 * u.deg), (0 * u.deg, 2.1 * u.deg)),
     ((0, 1), (1, 2), (2, 3)))
])
def test_for_time_interval_creates_correct_finder_charts(positions, expected_indices):
    ...
    ephemerides = [ephemeris(epoch=t + timedelta(hours=i), ra=positions[i][0], dec=positions[i][1]) for i, _ in enumerate(positions)]
    ...
    g = FinderChart.for_time_interval(start, end, ephemerides, 1 * u.deg, cast(Any, fake_create_finder_chart))
