...
from imephu.utils import Ephemeris, MagnitudeRange, mid_position, \
...
@pytest.mark.parametrize("start,end,expected", [
    (SkyCoord(ra=10 * u.deg, dec=0 * u.deg), SkyCoord(ra=15 * u.deg, dec=0 * u.deg), SkyCoord(ra=12.5 * u.deg, dec=0 * u.deg)),
    (SkyCoord(ra=130 * u.deg, dec=-20 * u.deg), SkyCoord(ra=130 * u.deg, dec=-40 * u.deg), SkyCoord(ra=130 * u.deg, dec=-30 * u.deg))
])
...
@pytest.mark.parametrize("bandpasses,expectation", [
    ((None, None), _does_not_raise()), ((None, "V"), _does_not_raise()), (("V",), _does_not_raise()), (("R", "R"), _does_not_raise()), (("V", "R", "V"), pytest.raises(ValueError))
])
def test_ephemerides_magnitude_range_requires_consistent_bandpass(bandpasses, expectation):
    """Test that the bandpass must be the same for every ephemeris."""
    ephemerides = []
    for bandpass in bandpasses:
        if bandpass:
            mr: Optional[MagnitudeRange] = MagnitudeRange(min_magnitude=17.1, max_magnitude=17.2, bandpass=bandpass)
        else:
            mr = None
        ephemerides.append(Ephemeris(epoch=datetime(2022, 2, 19, 0, 0, 0, 0, tzinfo=timezone.utc), position=SkyCoord(ra=0 * u.deg, dec=0 * u.deg), magnitude_range=mr))

    with expectation:
        ephemerides_magnitude_range(ephemerides)

@pytest.mark.parametrize("min_magnitudes,max_magnitudes,expected", [
    ((16.9,), (16.9,), MagnitudeRange(min_magnitude=16.9, max_magnitude=16.9, bandpass="B")),
    ((16.9,), (17.0,), MagnitudeRange(min_magnitude=16.9, max_magnitude=17.0, bandpass="B")),
...
def test_ephemerides_magnitude_range(min_magnitudes, max_magnitudes, expected):
    ...
    for i, _ in enumerate(min_magnitudes):
        if min_magnitudes[i] is not None:
            mr: Optional[MagnitudeRange] = MagnitudeRange(min_magnitude=min_magnitudes[i], max_magnitude=max_magnitudes[i], bandpass="B")
        else:
            mr = None
        ephemerides.append(Ephemeris(epoch=datetime(2022, 2, 19, 0, 0, 0, 0, tzinfo=timezone.utc), position=SkyCoord(ra=0 * u.deg, dec=0 * u.deg), magnitude_range=mr))
